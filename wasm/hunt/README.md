# wasm/hunt — the ranged engine

`apps/hunt/assets/hunt.wasm` is a **vendored binary**, built once offline and committed like a
codec. Same recipe and same flags as the brick engine this forked from — read that first;
this file is only the delta.

```sh
bash wasm/hunt/build.sh      # → apps/hunt/assets/hunt.wasm
```

## Forked, not rewritten

It starts from the brick engine's `game.c` and therefore keeps every fix already paid for there: the
